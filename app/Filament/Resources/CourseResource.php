            ...
            Forms\Components\Section::make('Dati corso')
                ->schema([
                    ...
                    Forms\Components\Textarea::make('description')
                        ->label('Descrizione')->rows(4)->columnSpanFull(),
                    Forms\Components\Grid::make(3)->schema([
                        Forms\Components\TextInput::make('lessons_count')
                            ->label('Numero lezioni')->numeric()->minValue(0)->required()->default(0),
                        Forms\Components\TextInput::make('course_price')
                            ->label('Prezzo corso (euro)')->numeric()->required()->default(0),
                        Forms\Components\TextInput::make('enrollment_fee')
                            ->label('Tassa iscrizione (euro)')->numeric()->required()->default(0),
                    ]),
                ]),
        ]);
    }
